...
### Переменные окружения (OrangeHRM)

Для запуска UI-тестов OrangeHRM и тестов авторизации нужны учётные данные демо-пользователя. Задайте переменные окружения (локально — через `.env` в корне проекта, в CI — через секреты):

| Переменная | Описание |
|------------|----------|
| `ORANGEHRM_ADMIN_LOGIN` | Логин администратора |
| `ORANGEHRM_ADMIN_PASSWORD` | Пароль администратора |

Файл `.env` добавлен в `.gitignore`. Пример структуры — в `.env.example` (скопируйте в `.env` и подставьте свои значения). Чтобы подхватить переменные из `.env` при запуске тестов, установите `dotenv` и подключите его в начале `playwright.config.js` (`require('dotenv').config()`), либо экспортируйте переменные в терминале вручную.
